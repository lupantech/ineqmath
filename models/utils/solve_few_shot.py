...
class ProblemSolver:
    ...
    def build_query(self, problem: str, prob_type: str, task_prompt: str = "", shot_num: int = 0, solution_file_path: str = None) -> str:
        if prob_type == "bound":
        ...
        if solution_file_path:
            try:
                ...
                # Filter solution examples by type to match the current problem's type
                for (solution_key, solution_data) in solution_set.items():
                    if idx >= shot_num:
                        break
                    # Check if the solution type matches the current problem type
                    if solution_data.get('type') == prob_type:
                        solution_examples += f"\n\nProblem: {solution_data['problem']}\n\n"
                        solution_examples += f"Solution: {solution_data['solution']}\n\n"
                        idx += 1
            except Exception as e:
                print(f"Error loading solution set: {e}")

        query = f"{task_hint}{task_prompt}{solution_examples}Problem: {problem}\n\nSolution:"
        return query
...
"""
    ...
    def process_problem(self, data: Dict[str, Any], output_dir: str, task_prompt: str, shot_num: int, **kwargs) -> None:
        ...
        try:
            ...
            query = self.build_query(problem, prob_type, task_prompt, shot_num, solution_file_path)
...
if __name__ == "__main__":
    ...
    try:    
        ...
        kwargs = {"max_tokens": args.max_tokens, "solution_file_path": args.solution_file_path}
